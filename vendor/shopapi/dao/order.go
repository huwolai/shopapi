...
func (self *Order) OrdersWithPordYyg(prodId string,pIndex uint64,pSize uint64) ([]*OrderYyg,int64,error)  {
	...
	_,err :=db.NewSession().SelectBySql("select c.ydgy_name,c.mobile,a.codes as buy_code,b.* from `order_item_purchase_codes` a left join `order` b on a.`no`=b.`no` left join `account` c on c.`open_id`=b.`open_id` where  a.`prod_id` = ? limit ?,?",prodId,(pIndex-1)*pSize,pSize).LoadStructs(&orders)

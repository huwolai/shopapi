...
func (self *ProductDetail) ProductListWithCategoryIsLimit(appId string,categoryId int64,flags []string,noflags []string,pageIndex uint64,pageSize uint64) ([]*ProductDetail,int , error)  {
	...
	builder :=session.Select("product.*,UNIX_TIMESTAMP(product.create_time) as create_time_unix,merchant.id merchant_id,merchant.name merchant_name").From("product").Join("prod_category","product.id = prod_category.prod_id").Join("merchant_prod","product.id = merchant_prod.prod_id").Join("merchant","merchant.id = merchant_prod.merchant_id").Join("prod_purchase_codes","product.id = prod_purchase_codes.prod_id").Where("prod_category.category_id=?",categoryId).Where("product.status=?",1).Where("product.parent_id=?",0).Where("product.app_id=?",appId)
	if flags!=nil&&len(flags)>0{
		builder = builder.Where("product.flag in ?",flags)
	}
	if noflags!=nil&&len(noflags) >0 {
...
func (self *ProductDetail) ProductListSearch(appId string,prodName string,flags []string,noflags []string,pageIndex uint64,pageSize uint64) ([]*ProductDetail,int , error)  {
	session := db.NewSession()
	var prodList []*ProductDetail
	var count int
	...
	buildercount=buildercount.From("product").Join("merchant_prod","product.id = merchant_prod.prod_id").Join("merchant","merchant.id = merchant_prod.merchant_id").Where("product.title  like ?","%"+prodName+"%").Where("product.status=?",1).Where("product.parent_id=?",0).Where("product.app_id=?",appId)

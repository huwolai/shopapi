...
func (self *ProdAttrVal) WithAttrKeyStock(vsearch string,attrKey string,prodId int64)  ([]*ProdAttrVal,error) {
	...
	_,err :=db.NewSession().SelectBySql("select pv.* from prod_attr_val pv,prod_sku ps where pv.prod_id=ps.prod_id and pv.id=ps.attr_symbol_path and ps.stock=0 and pv.attr_key=? and pv.prod_id=? and pv.attr_value like '?%'",attrKey,prodId).LoadStructs(&prodAttrvals)
